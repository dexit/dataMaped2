...
const CsvImportModal: React.FC<{ isOpen: boolean; onClose: () => void; onConfirm: (config: {name: string, category: string, datamap: DatamapEntry[]}) => void; csvData: { headers: string[], rows: string[][] } | null; categories: Category[]; showToast: (message: string, type: 'success' | 'error') => void; initialName: string; }> = ({ isOpen, onClose, onConfirm, csvData, categories, showToast, initialName }) => {
    ...
    const renderSelect = (field: keyof ColumnMap) => <select value={columnMap[field]} onChange={e => setColumnMap(prev => ({...prev, [field]: e.target.value}))} className="block w-full rounded-md border-slate-300 shadow-sm focus:border-emerald-500 focus:ring-emerald-500 sm:text-sm"><option value="">-- Not Mapped --</option>{csvData.headers.map(h => <option key={h} value={h}>{h}</option>)}</select>;
    ...
    return ( <Modal isOpen={isOpen} onClose={onClose} title="Import from CSV" size="xl" footer={footer}><div className="space-y-6"><div className="grid grid-cols-1 md:grid-cols-2 gap-4"><div><label className="block text-sm font-medium text-slate-700">Mapping Name</label><input type="text" value={mappingName} onChange={e => setMappingName(e.target.value)} className="mt-1 block w-full rounded-md border-slate-300 shadow-sm focus:border-emerald-500 focus:ring-emerald-500 sm:text-sm" /></div><div><label className="block text-sm font-medium text-slate-700">Category</label><select value={category} onChange={e => setCategory(e.target.value)} className="mt-1 block w-full rounded-md border-slate-300 shadow-sm focus:border-emerald-500 focus:ring-emerald-500 sm:text-sm"><option value="">None</option>{categories.map(c => <option key={c.id} value={c.name}>{c.name}</option>)}</select></div></div><div><h3 className="text-lg font-medium text-slate-800">Map CSV Columns</h3><div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-2"><div><label className="block text-sm font-medium text-slate-700">Source Field</label>{renderSelect('sourceField')}</div><div><label className="block text-sm font-medium text-slate-700">Source Type</label>{renderSelect('sourceType')}</div><div><label className="block text-sm font-medium text-slate-700">Target Field</label>{renderSelect('targetField')}</div><div><label className="block text-sm font-medium text-slate-700">Target Type</label>{renderSelect('targetType')}</div></div></div><div><h3 className="text-lg font-medium text-slate-800">Data Preview (first 5 rows)</h3><div className="overflow-x-auto mt-2 border border-slate-200 rounded-lg"><table className="min-w-full text-sm"><thead className="bg-slate-100"><tr className="text-left">{csvData.headers.map(h => <th key={h} className="p-2 font-semibold text-slate-600">{h}</th>)}</tr></thead><tbody className="bg-white divide-y divide-slate-200">{csvData.rows.slice(0, 5).map((row, i) => <tr key={i}>{row.map((cell, j) => <td key={j} className="p-2 truncate text-slate-700">{cell}</td>)}</tr>)}</tbody></table></div></div></div></Modal> )
};
...
const MappingFormModal: React.FC<{ isOpen: boolean; onClose: () => void; onSave: (data: {name: string, category: string}) => void; mapping: Pick<Mapping, 'name' | 'category'> | null; categories: Category[]; showToast: (message: string, type: 'success' | 'error') => void; }> = ({isOpen, onClose, onSave, mapping, categories, showToast}) => {
    ...
    return ( <Modal isOpen={isOpen} onClose={onClose} title={mapping ? "Edit Mapping" : "Add New Mapping"} size="md" footer={footer}><div className="space-y-4"><div><label className="text-sm font-medium text-slate-700">Mapping Name</label><input type="text" placeholder="e.g., User Profile V2" value={formData.name} onChange={e => setFormData(p => ({ ...p, name: e.target.value }))} className="mt-1 block w-full rounded-md border-slate-300 shadow-sm focus:border-emerald-500 focus:ring-emerald-500"/></div><div><label className="text-sm font-medium text-slate-700">Category (optional)</label><select value={formData.category} onChange={e => setFormData(p => ({ ...p, category: e.target.value }))} className="mt-1 block w-full rounded-md border-slate-300 shadow-sm focus:border-emerald-500 focus:ring-emerald-500"><option value="">None</option>{categories.map(c => <option key={c.id} value={c.name}>{c.name}</option>)}</select></div></div></Modal> );
};
...
const MappingManager: React.FC<MappingManagerProps> = ({ mappings, setMappings, categories, showToast, showConfirmation }) => {
  ...
  const renderAddEntryRow = (mappingId: string) => (
    <tr className="bg-slate-50">
        <td className="p-2"><input type="text" placeholder="Source Field" value={newDatamapEntry.sourceField} onChange={e => setNewDatamapEntry({...newDatamapEntry, sourceField: e.target.value})} className="w-full text-sm rounded-md border-slate-300 shadow-sm focus:border-emerald-500 focus:ring-emerald-500" /></td>
        <td className="p-2"><select value={newDatamapEntry.sourceType} onChange={e => setNewDatamapEntry({...newDatamapEntry, sourceType: e.target.value})} className="w-full text-sm rounded-md border-slate-300 shadow-sm focus:border-emerald-500 focus:ring-emerald-500">{DATA_TYPES.map(t => <option key={t} value={t}>{t}</option>)}</select></td>
        <td className="p-2"><input type="text" placeholder="Target Field" value={newDatamapEntry.targetField} onChange={e => setNewDatamapEntry({...newDatamapEntry, targetField: e.target.value})} className="w-full text-sm rounded-md border-slate-300 shadow-sm focus:border-emerald-500 focus:ring-emerald-500" /></td>
        <td className="p-2"><select value={newDatamapEntry.targetType} onChange={e => setNewDatamapEntry({...newDatamapEntry, targetType: e.target.value})} className="w-full text-sm rounded-md border-slate-300 shadow-sm focus:border-emerald-500 focus:ring-emerald-500">{DATA_TYPES.map(t => <option key={t} value={t}>{t}</option>)}</select></td>
    ...
    if (isEditing && editingEntryData) {
        return (
            <tr key={entry.id} className="bg-emerald-50">
                <td className="p-2"><input type="text" value={editingEntryData.sourceField} onChange={e => setEditingEntryData({...editingEntryData, sourceField: e.target.value})} className="w-full text-sm rounded-md border-emerald-300" /></td>
                <td className="p-2"><select value={editingEntryData.sourceType} onChange={e => setEditingEntryData({...editingEntryData, sourceType: e.target.value})} className="w-full text-sm rounded-md border-emerald-300">{DATA_TYPES.map(t=><option key={t} value={t}>{t}</option>)}</select></td>
                <td className="p-2"><input type="text" value={editingEntryData.targetField} onChange={e => setEditingEntryData({...editingEntryData, targetField: e.target.value})} className="w-full text-sm rounded-md border-emerald-300" /></td>
                <td className="p-2"><select value={editingEntryData.targetType} onChange={e => setEditingEntryData({...editingEntryData, targetType: e.target.value})} className="w-full text-sm rounded-md border-emerald-300">{DATA_TYPES.map(t=><option key={t} value={t}>{t}</option>)}</select></td>
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row items-center gap-4 p-4 bg-white rounded-lg shadow-sm border border-slate-200">
        <div className="relative flex-grow w-full md:w-auto">
            ...
            <input type="text" placeholder="Search mappings..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="w-full pl-10 pr-4 py-2 rounded-md border-slate-300 shadow-sm focus:border-emerald-500 focus:ring-emerald-500"/>
        </div>
